...
graph — unmodified — once per COI, in a plain Python loop, then merges the
...
def main() -> None:
    ...
    # judgment genuinely benefits from the stronger model), but effort
    # dropped to "medium" rather than the default "high". Reading
    # eligibility.py's own system prompt: this node's task is a bounded
    # per-record include/exclude classification (structurally the same
    # shape as screen.py's task, just on fuller text) — NOT the open-ended
    # cross-record reasoning evidence.py does. "high" effort was spending
    # wall-clock on deliberation this task doesn't need; at ~20+ batches
    # per COI that was the dominant cost in a run that ran over an hour
    # without finishing. evidence_llm below keeps effort="high" (its
    # default) since whole-corpus tiering/gap-finding is exactly the kind
    # of task that benefits from it.
    eligibility_llm = make_lilly_analytical_dispatcher(effort="medium")
    ...
    for coi in COIS:
        ...
        try:
            ...
            # take down the other four. A live run hit exactly this: a
            # single httpx.ReadTimeout on evidence's streamed call for one
            # COI killed the whole script, discarding screen+eligibility
            # results already completed (and paid for) on prior COIs.
            log.error("=== COI %s FAILED: %s: %s ===", coi, type(e).__name__, e)
            failed_cois[coi] = f"{type(e).__name__}: {e}"
            _write_output()  # save whatever's completed so far before moving on
            continue

        per_coi_results[coi] = result
        _write_output()  # incremental save — every successful COI is persisted immediately
